...
function agregarCampoCorreo() {
  const emailContainer = document.getElementById("emailContainer");
  const correoCount =
    emailContainer.querySelectorAll(".email-input-group").length;
  ...
  emailGroup.innerHTML = `
            <div class="mb-2">
                <label for="${nuevoId}" class="form-label">Correo electrónico ${
    correoCount + 1
  }</label>
                <div class="input-group">
                    ...
                    ${
                      correoCount > 0
                        ? '<button class="btn btn-outline-danger remove-email" type="button"><i class="bi bi-trash"></i></button>'
                        : ""
                    }
                </div>
            </div>
...
function agregarEventosBotones() {
  ...
  botonesEditar.forEach((boton) => {
    boton.addEventListener("click", async function () {
      ...
      const pedidoID = this.dataset.id; // Obtener el ID del pedido
      try {
        /*const cancelado = await pedidoCancelado();
        if (!cancelado) {*/
  ...
  botonesWhats.forEach((boton) => {
    boton.addEventListener("click", async function () {
      ...
      try {
        ...
        Swal.fire({
          title: "Aviso",
          text: "Hubo un problema al enviar por WhatsApp",
          icon: "error",
          confirmButtonText: "Entendido",
        });
      }
...
function datosPedidos(limpiarTabla = true) {
  ...
  if (filtro) filtro.value = filtroFecha;

  const pedidosTable = document.getElementById("datosPedidos");
  if (!pedidosTable) {
    console.log("No se encontró el elemento con id 'datosPedidos'");
    return;
  }
