            ...
            try {
                ...
                if (response.success && response.data && response.data.length > 0) {
                    ...
                    response.data.forEach((reporte) => {
                        // Si tu backend manda "CARGO" y "ABONO" (no "CARGOS/ABONOS"), cámbialo en el destructuring:
                        const cargo = Number(reporte.CARGO || 0);
                        const abono = Number(reporte.ABONO || 0);
                        const saldo = Number(reporte.SALDO || 0);

                        const row = document.createElement("tr");
                        row.innerHTML = `
                            <td style="display: table-cell">${reporte.CLAVE || ""}</td>
                            <td>${reporte.TIPO || ""}</td>
                            ...
                            <td style="text-align:right;">
                                ${cargo !== 0 ? cargo.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' }) : ""}
                            </td>
                            <td style="text-align:right;">
                                ${abono !== 0 ? abono.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' }) : ""}
                            </td>
                            <td style="text-align:right;">
