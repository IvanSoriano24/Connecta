...
// Variable global para almacenar las partidas
let partidasData = [];

// Maneja la creaciÃ³n de la fila de partidas
function agregarFilaPartidas() {
  ...
  if (filas.length > 0) {
    ...
    // No hay validación de totales para muestras
    ...
    // Validación: solo se necesita producto y cantidad > 0 para muestras
  }

  // Crear un identificador Ãºnico para la partida
  ...
  nuevaFila.innerHTML = `
      ...
      <td><input type="number" class="cantidad" value="0" style="text-align: right;" /></td>
  ...
  // AÃ±adir evento al botÃ³n de eliminar con delegaciÃ³n
  ...
  cantidadInput.addEventListener("input", () => {
    if (parseFloat(cantidadInput.value) < 0) {
      ...
      //calcularSubtotal(nuevaFila); // Recalcular subtotal si el valor es vÃ¡lido
    }
  });
  
  // Las muestras no tienen campo de descuento, así que omitimos esa lógica
  ...
  //console.log("Partidas actuales despuÃ©s de agregar:", partidasData);
}
...
function eliminarPartidaFormulario(numPar, filaAEliminar) {
  ...
  Swal.fire({
    ...
    text: "Â¿Deseas eliminar esta partida?",
    icon: "warning",
...
function obtenerProductos(input) {
  // numFuncion=5 indica en PHP que queremos la acciÃ³n â€œobtener productosâ€
  const numFuncion = 5;
  const chechk = false;
  //alert(chechk);
  ...
  xhr.open(
    "GET",
    "../Servidor/PHP/muestras.php?numFuncion=" + numFuncion + "&chechk=" + chechk,
    true
  );
  ...
  xhr.onload = function () {
    // Si el servidor respondiÃ³ con HTTP 200 (OK)
    if (xhr.status === 200) {
      // Parseamos el texto JSON que devolviÃ³ el servidor
      ...
      if (response.success) {
        ...
        // Si success = false, verificar si es error de conexión/empresa para no mostrar alert
        if (response.message && (response.message.includes('conexión') || response.message.includes('empresa') || response.message.includes('No se encontró'))) {
          console.warn("Error al obtener productos:", response.message);
          // No mostrar alert para errores de conexión
          return;
        }
  ...
  // Si ocurre algÃºn error de red (por ejemplo, sin conexiÃ³n)
  xhr.onerror = function () {
    Swal.fire({
      title: "Aviso",
      text: "Hubo un problema con la conexiÃ³n.",
      icon: "error",
      confirmButtonText: "Aceptar",
    });
  };

  // Finalmente, enviamos la peticiÃ³n al servidor
  xhr.send();
}
function mostrarTodosProductos(element) {
  ...
  xhr.onload = function () {
    ...
    if (xhr.status === 200) {
      ...
      // Si el servidor indicÃ³ Ã©xito en el JSON:
      if (response.success) {
        // Llamamos a la funciÃ³n que muestra la lista de productos,
        // pasÃ¡ndole el array de productos y el input donde se debe desplegar
...
function mostrarListaProductosCheck(productos) {
      ...
      fila.onclick = async function () {
        ...
        if (existenciasReal > 0) {
          ...
          /*if (!producto.CVE_PRODSERV) {
            ...
            return; // ðŸš¨ Salir de la funciÃ³n si `filaProd` no es vÃ¡lido
          }
          ...
          // Desbloquear o mantener bloqueado el campo de cantidad segÃºn las existencias
  ...
  // Evento para actualizar la tabla al escribir en el campo de bÃºsqueda
...
async function obtenerImpuesto(cveEsqImpu) {
  return new Promise((resolve, reject) => {
    $.ajax({
      url: "../Servidor/PHP/muestras.php", // Endpoint en el servidor que procesa la solicitud
      type: "POST", // MÃ©todo HTTP POST
      data: { cveEsqImpu: cveEsqImpu, numFuncion: "7" }, // ParÃ¡metros enviados al servidor
      success: function (response) {
        try {
          // Si el servidor indica Ã©xito, extraemos los impuestos y resolvemos la promesa
          if (response.success) {
      ...
      error: function (xhr, status, error) {
        // Si falla la peticiÃ³n AJAX, rechazamos la promesa
...
async function completarPrecioProducto(cveArt, filaTabla) {
  try {
    ...
    // Llamamos a la funciÃ³n que obtiene precio y lista usada
    const resultado = await obtenerPrecioProducto(cveArt, cvePrecio);
    if (!resultado) return; // Si no hay resultado vÃ¡lido, salimos
    ...
    if (precioInput) {
      ...
      console.error("No se encontrÃ³ el campo 'precioUnidad' en la fila.");
    }
    ...
    if (!CVE_ESQIMPU) {
      console.error("CVE_ESQIMPU no tiene un valor vÃ¡lido.");
      return;
    }
    // Llamamos a la funciÃ³n que obtiene los valores de impuestos
    ...
    if (impuesto1Input && impuesto4Input) {
      ...
      console.error(
        "No se encontraron uno o mÃ¡s campos de impuestos en la fila."
      );
    }

    // AquÃ­ podrÃ­as manejar impuesto2Input si lo usas en alguna lÃ³gica adicional
  } catch (error) {
...
async function obtenerPrecioProducto(claveProducto, listaPrecioCliente) {
  try {
    ...
    const response = await $.get("../Servidor/PHP/muestras.php", {
      numFuncion: "6",
...
function mostrarProductos(input) {
  // Abre el modal de productos automÃ¡ticamente
  ...
  // Llamar a la funciÃ³n AJAX para obtener los productos desde el servidor
...
function consolidarPartidasEnTabla() {
  ...
  $("#tablaProductos tbody tr").each(function () {
    ...
    if (!mapa[producto]) {
      ...
      // Ya existÃ­a: sumamos la cantidad y removemos esta fila
  ...
  Object.values(mapa).forEach(({ fila, cantidadAcumulada }) => {
    // Actualizar cantidad en la primera fila
...
function guardarMuestra(id) {
  try {
    ...
    if (!validacion) {
      Swal.fire({
        title: "Error al guardar el Muestra",
    ...
    Swal.fire({
      ...
      text: "Por favor, espera mientras se completa el Muestra.",
    ...
    console.error("Error en guardarMuestra:", error);
  }
}
function validarDatosEnvio() {
  //Obtenemos los valores
  const nombreContacto = document.getElementById("nombreContacto")?.value || '';
  const companiaContacto = document.getElementById("compaÃ±iaContacto")?.value || '';
...
function obtenerDatosFormulario() {
  ...
  // Convertir fecha DD/MM/YYYY a formato de base de datos YYYY-MM-DD
  const diaAltaInput = document.getElementById("diaAlta").value;
  let diaAlta = "";
  if (diaAltaInput) {
    const partes = diaAltaInput.split('/');
    if (partes.length === 3) {
      diaAlta = `${partes[2]}-${partes[1]}-${partes[0]} 00:00:00.000`;
    }
  } else {
    diaAlta = now.toISOString().slice(0, 10).replace("T", " ") + " 00:00:00.000";
  }

  // Convertir fecha de entrega DD/MM/YYYY a formato YYYY-MM-DD
  const campoEntrega = document.getElementById("entrega").value;
  let entrega = "";
  if (campoEntrega) {
    const partesEntrega = campoEntrega.split('/');
    if (partesEntrega.length === 3) {
      entrega = `${partesEntrega[2]}-${partesEntrega[1]}-${partesEntrega[0]}`;
    }
  }
  const formularioData = {
    ...
    ordenCompra: document.getElementById("suMuestra").value,
...
function extraerDatosEnvio() {
  ...
  const envioData = {
    ...
    companiaContacto: document.getElementById("compaÃ±iaContacto")?.value || '',
    diaAlta: diaAlta, // Fecha y hora
    fechaAlta: fechaActual, // Fecha y hora
...
function obtenerDatosPartidas() {
  ...
  filas.forEach((fila) => {
    ...
    // Solo agregar si hay producto y cantidad > 0
    if (producto.trim() !== "" && parseFloat(cantidad) > 0) {
      const partida = {
        CVE_ART: producto.trim(),
        CANT: parseFloat(cantidad),
        UNI_VENTA: fila.querySelector(".unidad")?.value || "",
        CVE_UNIDAD: fila.querySelector(".CVE_UNIDAD")?.value || "",
        CVE_PRODSERV: fila.querySelector(".CVE_PRODSERV")?.value || "",
        COSTO_PROM: fila.querySelector(".COSTO_PROM")?.value || "0"
      };
...
async function enviarDatosBackend(formularioData, partidasData, envioData) {
  ...
  for (const del of eliminacionesPendientes) {
    ...
    fdDel.append("claveMuestra", String(del.claveMuestra));
    fdDel.append("numPar", String(del.numPar));

    const r = await fetch("../Servidor/PHP/muestras.php", {
      method: "POST",
      body: fdDel,
    });
    const j = await r.json();
    if (!j || j.success !== true) {
      // Si falla una eliminaciÃ³n, avisa y conserva en cola para reintentar si quieres
  ...
  const tipoOperacion = document.getElementById("tipoOperacion").value || "alta";
  //Se crear un FormData para enviar los datos
  const formData = new FormData();
  const url = "../Servidor/PHP/muestras.php";
  
  // Preparar datos para muestras (sin crédito, siempre numFuncion 8)
  formData.append("numFuncion", "8");
  
  // Preparar datos del formulario para muestras
  const datosFormularioMuestra = {
    clienteClave: formularioData.cliente || "",
    nombreCliente: formularioData.nombre || "",
    fecha: formularioData.diaAlta || "",
    fechaEntrega: formularioData.entrega || formularioData.diaAlta || "",
    observaciones: formularioData.observaciones || "",
    vendedor: formularioData.claveVendedor || document.getElementById("vendedor").value || "1"
  };
  
  // Preparar partidas en formato correcto
  const partidasFormateadas = partidasData.map(p => ({
    CVE_ART: p.CVE_ART || p.producto || "",
    CANT: p.CANT || p.cantidad || 0,
    UNI_VENTA: p.UNI_VENTA || p.unidad || "",
    descripcion: "" // Se puede obtener del backend si es necesario
  }));
  
  // Combinar todos los datos en un solo objeto para el backend
  const datosCompletos = {
    ...datosFormularioMuestra,
    partidas: partidasFormateadas,
    envioData: envioData
  };
  
  // Se envían los datos del formulario (el backend espera 'datos' y 'accion')
  formData.append("datos", JSON.stringify(datosCompletos));
  // Se especifica la acción
  formData.append("accion", tipoOperacion === "alta" ? "crear" : "editar");
  fetch(url, {
    method: "POST",
    body: formData,
  })
    .then((response) => {
      ...
      return response.text(); // Obtener la respuesta como texto para depuraciÃ³n
    })
    .then((text) => {
      ...
      try {
        ...
        throw new Error("El servidor no devolviÃ³ una respuesta invÃ¡lida.");
      }
    })
    .then((data) => {
      if (!data) return;
      ...
      if (data.success) {
        guardarOrden();
        Swal.fire({
          title: "Â¡Muestra guardado exitosamente!",
          html: `
      <p>${data.message || "El Muestra se procesÃ³ correctamente."}</p>
    ...
    `,
        ...
        Swal.fire({
          title: "Muestra guardado",
          html: `
      <p>${data.message || "El Muestra se enviÃ³ solo por correo."}</p>
      ${data.correo ? `<p><strong>Correo:</strong> ${data.correo}</p>` : ""}
    `,
          icon: "info",
          confirmButtonText: "Aceptar",
        ...
        Swal.fire({
          title: "Muestra guardado",
          html: `
      <p>${data.message || "El Muestra se enviÃ³ solo por WhatsApp."}</p>
      ${data.whats ? `<p><strong>WhatsApp:</strong> ${data.whats}</p>` : ""}
    `,
          icon: "info",
          confirmButtonText: "Aceptar",
        ...
        Swal.fire({
          title: "Muestra guardado",
          text: data.message || "El Muestra fue guardado pero no se enviaron notificaciones.",
          icon: "info",
          confirmButtonText: "Aceptar",
        }).then(() => {
          window.location.href = "Muestras.php";
        ...
        //Mensaje cuando se tiene que autorizar el Muestra por un administrador
        Swal.fire({
          title: "Saldo vencido",
          text:
            data.message || "El Muestra se procesÃ³ pero debe ser autorizado.",
          icon: "warning",
          confirmButtonText: "Entendido",
        }).then(() => {
          // Redirigir al usuario o realizar otra acciÃ³n
          guardarOrden();
          window.location.href = "Muestras.php";
        });
      }
...
function guardarOrden() {
  ...
  // Si tÃº guardas por AJAX, llama aquÃ­ tu funciÃ³n que guarda en Firestore
  // await guardarEnFirestore(...);
  // 4) Limpia selecciÃ³n temporal del modal
  window.MDPDFs?.reset?.();
}
async function subirSeleccionSiAplica() {
  ...
  // Sube AQUÃ (NO en el modal). Esto llama a tu enviarHistorico.php internamente.
...
// FunciÃ³n para abrir el modal y cargar los clientes
...
// FunciÃ³n para renderizar los clientes en la tabla
function renderClientes(clientes) {
  ...
  clientes.forEach((cliente) => {
    ...
    fila.innerHTML = `
            ...
            <td>${cliente.TELEFONO || "Sin telÃ©fono"}</td>
...
// FunciÃ³n para seleccionar un cliente desde el modal
function seleccionarClienteDesdeModal(cliente) {
  ...
  // Cargar datos de envío y estados cuando se selecciona cliente
  // Solo cargar si hay un cliente válido
  ...
  fetch(`../Servidor/PHP/clientes.php?clienteId=${clienteId}&numFuncion=3`)
    ...
    .then((data) => {
      if (data.success) {
        ...
        if (conCredito === "S") {
          Swal.fire({
            title: "Cliente vÃ¡lido",
            text: "El cliente tiene crÃ©dito disponible.",
            icon: "success",
          });
          $("#conCredito").val("S");
        } else {
          Swal.fire({
            title: "Sin crÃ©dito",
            text: "El cliente no maneja crÃ©dito.",
            icon: "info",
          });
          $("#conCredito").val("N");
        }
    ...
    .catch((error) => {
      ...
      Swal.fire({
        title: "Aviso",
        text: "OcurriÃ³ un error al validar el cliente.",
        icon: "error",
      });
    });
}

// FunciÃ³n para mostrar sugerencias de clientes
...
// FunciÃ³n para seleccionar un cliente desde las sugerencias
...
// FunciÃ³n para mostrar sugerencias de prodcuctos
...
// FunciÃ³n para seleccionar un produto desde las sugerencias
async function seleccionarProductoDesdeSugerencia(inputProducto, producto) {
  ...
  if (!filaProd) {
    console.error("Error: No se encontrÃ³ la fila del producto.");
    return; // ðŸš¨ Salir de la funciÃ³n si `filaProd` no es vÃ¡lido
  }
  ...
  // Ocultar sugerencias despuÃ©s de seleccionar
...
function desbloquearCampos() {
  $(
    "#entrega, #suMuestra, #entrega, #condicion, #descuentofin, #datosEnvio, #observaciones, #enviarWhats, #enviarCorreo"
  ).prop("disabled", false);
}
function llenarDatosCliente(cliente) {
  ...
  // Validar el crÃ©dito del cliente
  validarCreditoCliente(cliente.CLAVE);
}
// Filtrar clientes segÃºn la entrada de bÃºsqueda en el modal
...
function obtenerEstados() {
  ...
  $.ajax({
    ...
    success: function (resEstado) {
      // Si hay un error de conexión/empresa, no mostrar alert, solo log
      if (!resEstado.success && resEstado.message && (resEstado.message.includes('conexión') || resEstado.message.includes('empresa') || resEstado.message.includes('No se encontró'))) {
        console.warn("Error al obtener estados:", resEstado.message);
        return;
      }
      ...
      if (resEstado.success && Array.isArray(resEstado.data)) {
        ...
        // No mostrar alert si es error de conexión, ya se manejó arriba
        if (resEstado.message && !resEstado.message.includes('conexión') && !resEstado.message.includes('empresa')) {
          Swal.fire({
            icon: "warning",
            title: "Aviso",
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      // No mostrar alert si es un error relacionado con conexión
      console.warn("Error AJAX al obtener estados:", textStatus, errorThrown);
      // Solo mostrar alert si no es un error de conexión
...
function obtenerMunicipios(edo, municipio) {
  ...
  $.ajax({
    ...
    success: function (resMunicipio) {
      // Si hay un error de conexión/empresa, no mostrar alert, solo log
      if (!resMunicipio.success && resMunicipio.message && (resMunicipio.message.includes('conexión') || resMunicipio.message.includes('empresa') || resMunicipio.message.includes('No se encontró'))) {
        console.warn("Error al obtener municipios:", resMunicipio.message);
        return;
      }
      ...
      if (resMunicipio.success && Array.isArray(resMunicipio.data)) {
        ...
        // Filtrar según el largo del RFC
        resMunicipio.data.forEach((municipioItem) => {
          $municipioNuevoContacto.append(
            `<option value="${municipioItem.Clave}" 
                data-estado="${municipioItem.Estado}"
                data-Descripcion="${municipioItem.Descripcion || ""}">
                ${municipioItem.Descripcion}
              </option>`
          );
        });
        if (municipio) {
        ...
        // No mostrar alert si es error de conexión, ya se manejó arriba
        if (resMunicipio.message && !resMunicipio.message.includes('conexión') && !resMunicipio.message.includes('empresa')) {
          Swal.fire({
            icon: "warning",
            title: "Aviso",
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      // No mostrar alert si es un error relacionado con conexión
      console.warn("Error AJAX al obtener municipios:", textStatus, errorThrown);
      // Solo mostrar alert si no es un error de conexión
      if (textStatus !== 'timeout' && textStatus !== 'error') {
        Swal.fire({
          icon: "error",
  ...
  $.ajax({
    ...
    success: function (resEstado) {
      if (resEstado.success && Array.isArray(resEstado.data)) {
        ...
        // Filtrar segÃºn el largo del RFC
        resEstado.data.forEach((estado) => {
          $estadoNuevoContacto.append(
            `<option value="${estado.Clave}" 
                data-Pais="${estado.Pais}">
...
function obtenerMunicipiosMuestra() {
  // Habilitamos el select
...
// Vincular los eventos de bÃºsqueda y cambio de criterio
// ==================== FUNCIÓN: OBTENER FOLIO SIGUIENTE ====================
async function obtenerFolioSiguiente() {
  try {
    const response = await fetch("../Servidor/PHP/muestras.php", {
      method: "POST",
      headers: {
    ...
    if (data.success && data.folio) {
      ...
      // Si el error es por empresa no encontrada, usar folio 1 como fallback sin mostrar mensaje
      if (data.message && (data.message.includes('empresa') || data.message.includes('conexión') || data.message.includes('No se encontró'))) {
        // Solo log en consola, no mostrar advertencia al usuario
        return 1;
      }
      throw new Error(data.message || "Error al obtener folio");
    }
  } catch (error) {
    console.error("Error en obtenerFolioSiguiente:", error);
    // En caso de error, retornar folio 1 como fallback en lugar de lanzar error
    return 1;
  }
}
...
function obtenerFecha() {
  ...
  const fechaInput = document.getElementById("diaAlta");
  if (fechaInput) {
    fechaInput.value = fechaActual;
  }
}

// ==================== FUNCIÓN: FORMATO FECHA DD/MM/YYYY ====================
...
function validarFecha(input) {
  ...
  if (fecha && !regex.test(fecha)) {
    // Si no está vacía y no cumple el formato, limpiar
    if (fecha.length === 10) {
      input.value = '';
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  // Configurar máscaras de fecha para inputs de fecha
  const fechaInput = document.getElementById("diaAlta");
  const entregaInput = document.getElementById("entrega");
  
  if (fechaInput) {
    fechaInput.addEventListener('input', () => formatearFecha(fechaInput));
    fechaInput.addEventListener('blur', () => validarFecha(fechaInput));
  }
  
  if (entregaInput) {
    entregaInput.addEventListener('input', () => formatearFecha(entregaInput));
    entregaInput.addEventListener('blur', () => validarFecha(entregaInput));
  }
  
  // Inicialización para nuevo muestra
  const urlParams = new URLSearchParams(window.location.search);
  const muestraID = urlParams.get("muestraID");

  if (!muestraID) {
    // Es un nuevo muestra
    sessionStorage.setItem("clienteSeleccionado", "false");
    obtenerFecha();
    obtenerFolioSiguiente().then((folio) => {
      const numeroInput = document.getElementById("numero");
      if (numeroInput) {
        numeroInput.value = folio;
      }
    }).catch((error) => {
      console.error("Error al obtener folio inicial:", error);
      // No mostrar alert si falla al obtener folio inicial, solo log en consola
      // El usuario podrá seguir trabajando y se intentará obtener el folio al guardar
    });
  } else {
    // TODO: Cargar datos de muestra existente si es edición
...
function obtenerDatosEnvio() {
  ...
  // Extraer solo la clave del cliente si viene con formato "CLAVE - NOMBRE"
  let clienteId = clienteInput.value.trim();
  if (clienteId.includes(" - ")) {
    clienteId = clienteId.split(" - ")[0].trim();
  }
  
  if (!clienteId || clienteId === "") {
    // No mostrar error si no hay cliente seleccionado aún
    return;
  }
  ...
  $.ajax({
    ...
    method: "GET",
    data: { numFuncion: "5", clave: clienteId }, // Llamar la funciÃ³n para obtener vendedores
    success: function (response) {
      try {
        const res =
          typeof response === "string" ? JSON.parse(response) : response;

        // Si hay un error en la conexión (empresa no encontrada), no mostrar alert, solo log
        if (!res.success && res.message && (res.message.includes('empresa') || res.message.includes('conexión') || res.message.includes('No se encontró'))) {
          console.warn("No se encontró la empresa al obtener datos de envío:", res.message);
          return;
        }
        ...
        console.error("Error al Procesar la Respuesta:", error);
        // No mostrar alert genérico si es error de empresa, ya se manejó arriba
        if (!res || (res.message && !res.message.includes('empresa') && !res.message.includes('conexión'))) {
          Swal.fire({
            icon: "error",
            title: "Error",
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      // No mostrar alert si es un error relacionado con conexión/empresa
      console.warn("Error AJAX al obtener datos de envío:", textStatus, errorThrown);
      // Solo mostrar alert si no es un error de conexión
      if (textStatus !== 'timeout' && textStatus !== 'error') {
        Swal.fire({
          icon: "error",
...
function guardarDatosEnvio() {
  ...
  // 2. Leer todos los campos del formulario de nuevo envÃ­o
  ...
  const compania = document.getElementById("compaÃ±iaNuevoContacto")?.value || '';
  ...
  if (
    ...
    return; // Abortamos si falta algÃºn campo
  }

  // 4. Enviar los datos al servidor vÃ­a AJAX
  $.ajax({
    url: "../Servidor/PHP/clientes.php", // Punto final en PHP que procesarÃ¡ el guardado
    method: "POST",
    data: {
      numFuncion: "6", // Identificador de la funciÃ³n en el servidor
      clienteId: clienteId, // ID del cliente al que pertenece esta direcciÃ³n
      tituloEnvio: tituloEnvio, // TÃ­tulo o alias de la direcciÃ³n
      nombreContacto: nombreContacto,
      compania: compania,
    ...
    dataType: "json",
    success: function (envios) {
      // Esta funciÃ³n se ejecuta si la peticiÃ³n AJAX devuelve HTTP 200
      if (envios.success) {
        Swal.fire({
          icon: "success",
          title: "Ã‰xito",
          text: "Se guardaron los nuevos datos de envÃ­o.",
        }).then(() => {
          // Cuando cierren el cuadro de alerta, ocultamos el modal y recargamos listados
          $("#modalNuevoEnvio").modal("hide");
          mostrarMoldal(); // Suponemos que esta funciÃ³n recarga la lista de envÃ­os
        });
      } else {
        ...
        Swal.fire({
          icon: "warning",
          title: "Aviso",
          text: envios.message || "No se pudieron guardar los datos de envÃ­o.",
        });
      }
    },
    error: function () {
      // Si la peticiÃ³n AJAX falla (500, 404, etc.)
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Error al guardar los datos de envÃ­o.",
      });
    },
  });
}
...
function limpiarFormulario() {
  ...
  $("#folioDatos").val("");
  $("#nombreContacto").val("");
  $("#compaÃ±iaContacto").val("");
  $("#telefonoContacto").val("");
  $("#correoContacto").val("");
  $("#direccion1Contacto").val("");
  $("#direccion2Contacto").val("");
  $("#codigoContacto").val(""); // Si es un select, tambiÃ©n se debe resetear
  $("#estadoContacto").val("");
...
function llenarDatosEnvio(idDocumento) {
  ...
  $.post(
    ...
    function (response) {
      // Si hay un error relacionado con empresa o conexión, solo log, no mostrar alert
      if (!response.success && response.message && (response.message.includes('empresa') || response.message.includes('conexión') || response.message.includes('No se encontró'))) {
        console.warn("No se encontró la empresa al llenar datos de envío:", response.message);
        return;
      }
      ...
      if (response.success && response.data) {
        ...
        console.log(data); // Esto te mostrarÃ¡ el objeto completo
        ...
        $("#compaÃ±iaContacto").val(data.compania.stringValue);
    ...
    // No mostrar error si es un problema de conexión/empresa
    console.warn("Error en la petición de llenar datos de envío:", textStatus, errorThrown);
    // Solo mostrar alert si no es un error relacionado con conexión
    if (textStatus !== 'timeout' && textStatus !== 'error') {
...
function obtenerTotales() {
  ...
  fetch("../Servidor/PHP/muestras.php", {
    method: "POST",
    body: form,
  })
    .then((res) => res.json())
    .then((data) => {
      ...
      const subtotalMuestra =
        data.subtotal.toFixed(2) - data.descuento.toFixed(2);
      // Â¡Ahora sÃ­ puedes leer data.subtotal, data.iva y data.importe!
      $("#subtotal").val(data.subtotal.toFixed(2));
      $("#descuento").val(data.descuento.toFixed(2));
      $("#subtotalMuestra").val(subtotalMuestra.toFixed(2));
...
const btnAñadirPartida = document.getElementById("añadirPartida");
if (btnAñadirPartida) {
  btnAñadirPartida.addEventListener("click", function () {
    agregarFilaPartidas();
  });
}
document
...
document
  .getElementById("formularioMuestra")
  .addEventListener("keydown", function (event) {
...
$(document).ready(function () {
  $("#guardarMuestra").click(async function (event) {
    //Funcion que se activa al guardar el Muestra
    event.preventDefault();
    const $btn = $(this);
    // Desactiva el botÃ³n al hacer clic
    $btn.prop("disabled", true);
    const clienteSeleccionado =
      sessionStorage.getItem("clienteSeleccionado") === "true";
    // Obtener el ID actual del Muestra desde el formulario
    let id = document.getElementById("numero").value;
    console.log("ID actual del Muestra:", id);

    try {
      // Obtener el siguiente folio del backend
      const folio = await obtenerFolioSiguiente();
      console.log("Folio obtenido:", folio);
      // Verificar si es una alta de Muestra o una edicion
      if (folio == id) {
        console.log("Guardando nuevo Muestra...");
        id = 0;
      } else {
        console.log("Editando Muestra existente...");
        document.getElementById("numero").value = id;
      }

      console.log(
        "NÃºmero final en el formulario:",
      ...
      if (ultimoProducto === "" || ultimaCantidad === 0) {
        ...
        Swal.fire({
          title: "Aviso",
          text: "Debes seleccionar un producto y una cantidad mayor a 0 antes de guardar el Muestra.",
          icon: "warning",
          confirmButtonText: "Entendido",
        });
        return;
      ...
      guardarMuestra(id);
      return false; // Evita la recarga de la pÃ¡gina
    } catch (error) {
      console.error("Error al obtener el folio:", error);
      return false; // Previene la recarga en caso de error
    } finally {
      $btn.prop("disabled", false); // â† Lo habilitas si hubo error inesperado
    }
  });
});
...
$("#AyudaCondicion").click(function () {
  ...
  Swal.fire({
    title: "Ayuda",
    text: "PodrÃ¡s capturar la condiciÃ³n bajo la cual se efectuarÃ¡ el pago del cliente (por ejemplo, Efectivo, CrÃ©dito, CÃ³brese o DevuÃ©lvase - C.O.D., etc.). ",
    icon: "info",
    confirmButtonText: "Entendido",
  });
});
...
$("#AyudaDescuento").click(function () {
  ...
  Swal.fire({
    title: "Ayuda",
    text: "Al momento de indicar la clave del cliente en el documento de venta, se muestra automÃ¡ticamente el porcentaje de descuento asignado a dicho cliente desde el MÃ³dulo de clientes. ",
    icon: "info",
    confirmButtonText: "Entendido",
  });
});
...
$("#AyudaDescuentofin").click(function () {
  ...
  Swal.fire({
    title: "Ayuda",
    text: "Un descuento financiero, es aquel que se otorga en circunstancias particulares (por Ejemplo, por pronto pago o por adquirir grandes volÃºmenes de mercancÃ­a). ",
    icon: "info",
    confirmButtonText: "Entendido",
  });
});
...
$("#cancelarMuestra").click(function () {
  //Boton para redigir a la seccion de Muestras
  window.location.href = "Muestras.php";
});
